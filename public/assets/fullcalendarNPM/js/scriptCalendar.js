    ...
    $.ajax({
        ...
        success: function (e) {
             if (e && (objCalendar.refetchEvents(), $("#modalCalendar").modal("hide")), !0 === showModalUpdateFastEvent) {
                ...
                let e = `{"id":"${t.id}","filiere":"${t.filiere}","title":"${t.title}","color":"${t.color}","start":"${t.start}","end":"${t.end}"},"etat":"${t.etat}"}`;
                $(`#boxFastEvent${t.id}`).attr("data-event", e),
                 $(`#boxFastEvent${t.id}`).text(t.title),
                  $(`#boxFastEvent${t.id}`).css({
            ...
            if (!0 === showModalCreateFastEvent) {
                ...
                let a = `{"id":"${e.created}","title":"${t.title}","color":"${t.color}","start":"${t.start}","end":"${t.end}"},"etat":"${t.etat}"}`,
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    var t = new e(document.getElementById("calendar"), {        
        ...
        eventClick: function (e) {
            ...
            clearMessages(".message"), resetForm("#formEvent"), $("#modalCalendar").modal("show"), $("#modalCalendar #titleModal").text("Modification : "+e.event.title), $("#modalCalendar button.deleteEvent").css("display", "flex"),$("#activation").show();
            let t = e.event.id;
            $("#modalCalendar input[name='id']").val(t);
            let a = e.event.title;
            $("#modalCalendar input[name='title']").val(a);
            let n = moment(e.event.start).format("DD/MM/YYYY HH:mm:ss");
            $("#modalCalendar input[name='start']").val(n);
            let o = moment(e.event.end).format("DD/MM/YYYY HH:mm:ss");
            $("#modalCalendar input[name='end']").val(o);
            
            let d = e.event.extendedProps.description;
            $("#modalCalendar textarea[name='description']").val(d);
            let type_event =e.event.extendedProps.type_event;
            if(type_event=='document'){ 
                $('#cours').show();
                document.getElementById('cour').value =e.event.extendedProps.cour;
                }else{ $("#salles").show(); $("#cours").hide();document.getElementById('salle').value =e.event.extendedProps.cour;}
             document.getElementById('type').value = type_event;
             let etat=e.event.extendedProps.etat;
        ...
        select: function (e) {
            ...
            clearMessages(".message"), resetForm("#formEvent"), $("#modalCalendar input[name='id']").val(""), $("#modalCalendar").modal("show"),
             $("#modalCalendar #titleModal").text("Ajouter nouveau programme"), $("#modalCalendar button.deleteEvent").css("display", "none"),$("#activation").hide();
